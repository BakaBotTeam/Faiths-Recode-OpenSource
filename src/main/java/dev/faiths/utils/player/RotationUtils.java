...
import dev.faiths.utils.math.Location;
...
public class RotationUtils {
    ...
    public static float[] getPlayerRotations(Entity entity) {
     ...
     * @param src          Ray trace origin {@link RotationUtils#getHitOrigin}
    ...
    public static Vector2f calculate(final dev.faiths.utils.vector.Vector3d from, final dev.faiths.utils.vector.Vector3d to) {

        final dev.faiths.utils.vector.Vector3d diff = to.subtract(from);
    ...
    public static Vector2f calculate(final Vec3 to) {
        return calculate(mc.thePlayer.getCustomPositionVector().add(0, mc.thePlayer.getEyeHeight(), 0), new dev.faiths.utils.vector.Vector3d(to.xCoord, to.yCoord, to.zCoord));
    }

    public static Vector2f calculate(final dev.faiths.utils.vector.Vector3d to) {
        return calculate(mc.thePlayer.getCustomPositionVector().add(0, mc.thePlayer.getEyeHeight(), 0), to);
    }

    public static Vector2f calculate(final dev.faiths.utils.vector.Vector3d position, final EnumFacing enumFacing) {
        ...
        return calculate(new dev.faiths.utils.vector.Vector3d(x, y, z));
    }
    ...
    public static float[] scaffoldRots(final double bx, final double by, final double bz, final float lastYaw, final float lastPitch, final float yawSpeed, final float pitchSpeed, final boolean random) {
        final double x = bx - RotationUtils.mc.thePlayer.posX;
        final double y = by - (RotationUtils.mc.thePlayer.posY + RotationUtils.mc.thePlayer.getEyeHeight());
        final double z = bz - RotationUtils.mc.thePlayer.posZ;
    ...
    public static float[] getHVHRotation(AxisAlignedBB axisAlignedBB, double maxRange) {
