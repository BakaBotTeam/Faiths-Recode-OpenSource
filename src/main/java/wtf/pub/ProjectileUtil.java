...
public class ProjectileUtil {
    public static ProjectileHit predict(double posX, double posY, double posZ, double motionX, double motionY, double motionZ, double motionSlowdown, double size, double gravity, boolean draw) {
        ...
        MovingObjectPosition before = null;
        ...
        while (!hasLanded && posY > -60.0D) {
            ...
            if (landingPosition != null) {
                ...
                before = new MovingObjectPosition(posBefore, landingPosition.sideHit);
            }
            ...
            posY += motionY;
            posZ += motionZ;
            travelDistance += posBefore.distanceTo(posAfter);
            ...
            motionY -= gravity;
            if (draw) {
                GL11.glVertex3d(posX - mc.getRenderManager().renderPosX, posY - mc
                        .getRenderManager().renderPosY, posZ - mc
                        .getRenderManager().renderPosZ);
            }
        }

        return new ProjectileHit(posX, posY, posZ, hitEntity, hasLanded, landingPosition, before, travelDistance);
    }
    ...
    public static class EnderPearlPredictor {
        ...
        private double assessSingleRotation(Vector2f rotation, double motionYOffset) {
            ...
            if (!canLandOn(landPos, projectileHit.beforeLandingPosition.hitVec)) {
                return 0.05D;
            }

            return ((facing == EnumFacing.UP || facing == EnumFacing.DOWN) ? assessPlainBlockPos(landPos) : assessSideBlockPos(landPos, facing)) * Math.pow(0.99, projectileHit.getTravelDistance());
        }

        private boolean canLandOn(BlockPos landPos, Vec3 before) {
            for (int i = 0; i < 10; i++) {
                if (!mc.theWorld.isAirBlock(landPos.add(before.xCoord, before.yCoord-i, before.zCoord))) {
                    return true;
                }
            }
    ...
    public static class ProjectileHit {
        ...
        private final MovingObjectPosition beforeLandingPosition;
        private final double travelDistance;

        public ProjectileHit(double posX, double posY, double posZ, boolean hitEntity,
                             boolean hasLanded, MovingObjectPosition landingPosition, MovingObjectPosition beforeLandingPosition, double travelDistance) {
            this.posX = posX;
            this.posY = posY;
            this.posZ = posZ;
            ...
            this.beforeLandingPosition = beforeLandingPosition;
