...
import dev.faiths.event.impl.RotationRenderEvent;
    ...
    public void doRender(T entity, double x, double y, double z, float entityYaw, float partialTicks)
    {
        if (!Reflector.RenderLivingEvent_Pre_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Pre_Constructor, new Object[] {entity, this, Double.valueOf(x), Double.valueOf(y), Double.valueOf(z)}))
        {
            ...
            try
            {
                ...
                RotationManager rotationManager = Faiths.INSTANCE.getRotationManager();
                if(rotationManager.targetRotation != null &&rotationManager.modify && entity == Minecraft.getMinecraft().thePlayer) {
                    f = this.interpolateRotation(RotationRenderEvent.prevRenderYawHead , RotationRenderEvent.renderYawHead , partialTicks);
                    f2 = this.interpolateRotation(RotationRenderEvent.prevRenderYawHead , RotationRenderEvent.renderYawHead , partialTicks) - f;
                    f7 = this.interpolateRotation(RotationRenderEvent.prevRenderPitchHead , RotationRenderEvent.renderPitchHead , partialTicks);
                }
