...
import dev.faiths.event.impl.SafeWalkEvent;
import dev.faiths.event.impl.StrafeEvent;
import dev.faiths.module.render.ModuleCamera;
import dev.faiths.utils.RiseSecret;
...
public abstract class Entity implements ICommandSender
{
    ...
    /**
     * The entity's X coordinate at the previous packet, used to calculate speed during rendering routines
     */
    public double lastPacketPosX;

    /**
     * The entity's Y coordinate at the previous packet, used to calculate speed during rendering routines
     */
    public double lastPacketPosY;

    /**
     * The entity's Z coordinate at the previous packet, used to calculate speed during rendering routines
     */
    public double lastPacketPosZ;
    ...
    public void setAngles(float yaw, float pitch) {
        ...
        ModuleCamera module = Faiths.moduleManager.getModule(ModuleCamera.class);
        if (!(module.getState() && module.getMcLock() && Mouse.isButtonDown(2))) {
    ...
    public void moveEntity(double x, double y, double z)
    {
        ...
        else
        {
            ...
            Faiths.INSTANCE.getEventManager().callEvent(safeWalkEvent);
    ...
    public float getClosestDistanceToEntity(Entity entityIn) {
        return getClosestDistanceToBBox(entityIn.getEntityBoundingBox());
    }

    public float getClosestDistanceToBBox(AxisAlignedBB boundingBox) {
        Vec3 eyes = this.getPositionEyes(1F);
        Vec3 pos = RotationUtils.getClosestPoint(eyes, boundingBox);
    ...
    public void moveFlying(float strafe, float forward, float friction) {
        ...
        Faiths.INSTANCE.getEventManager().callEvent(eventStrafe);
